// "Start.js"  11.01.2026 ----------
  ...
  let termineCache  = [];
  let rapporteCache = {}; // Struktur: { "2024": [...], "2025": [...] }
  let aktRapportJahr = new Date().getFullYear().toString(); // Standard: "2026"
...
// ----------------------------------
  if (alleAnmeldeDatenGeladen && alleMitarbeiterDatenGeladen && alleAnmeldeInfosGeladen && verfuegbareTermineGeladen && alleFunktionenGeladen && AktualisierungsStatusGeladen) {
    ...
    initialisiereRapportJahrDropdown();
  ...
  // 👇 Login-Overlay nach Laden sichtbar lassen
    if (isLoggedIn) {
      ...
      document.getElementById("overlayAdminLogin").style.display = "flex";
    }
...
// ----------------------------------
  ...
  try {
    ...
    TafelVersion.textContent = "Spreadsheet: " +TafelInfo;
  } catch (error) {
...
//-------------------------------------------------------------
function ladeRapporte(forceReload = false) {
//-------------------------------------------------------------
    const yearSelect = document.getElementById("rapport-year-select");
    
    // Falls gerufen durch onchange im Dropdown: globale Variable updaten
    if (yearSelect) {
        aktRapportJahr = yearSelect.value;
    }

    if (!forceReload && rapporteCache[aktRapportJahr]) {
        console.log(`🚀 Nutze Cache für Jahr: ${aktRapportJahr}`);
        filterRapporte();
        //zeigeRapporteTabelle(rapporteCache[aktRapportJahr]);
        return;
    }

    console.log(`☁️ Lade Daten vom Server für Jahr: ${aktRapportJahr}`);
    updateExportStatus('rapportContainer', `Lade Rapporte ${aktRapportJahr}...`, true);

    apiCall('getTagesrapporteList', { year: aktRapportJahr })
        .then(rapporte => {
            // Im Cache unter dem Jahr speichern
            rapporteCache[aktRapportJahr] = rapporte; 
            filterRapporte();
            //zeigeRapporteTabelle(rapporteCache[aktRapportJahr]);
        })
        .catch(error => {
            console.error("Fehler:", error);
            showPopup(`Fehler beim Laden für ${aktRapportJahr}`);
        })
        .finally(() => {
...
//-------------------------------------------------------------
function ladeTafelWochentagValue() {
//-------------------------------------------------------------

    apiCall('getTafelWochentagValue', {})
        .then(wochentag => {
      console.log("*********************ladeTafelWochentagValue: ", wochentag);
            TafelTag = wochentag; 
    })
        .catch(error => {
         console.error("Fehler beim Laden des TafelWochentags:", error);
    });
}
