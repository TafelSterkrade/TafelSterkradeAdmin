...
//-----------------------------------------------
    ...
    .then(response => {
        ...
        if (exportType === 'pdf') {
            ...
            exportRapport2XlsxAndSave(rapportname);
            ladeRapporte(true);
        }
    })
    .catch(error => {
...
//-----------------------------------------------
function exportRapport2XlsxAndSave(rapportname) {
//-----------------------------------------------
    const sheetName = "Tagesrapport";
    const filename = rapportname + ".xlsx";

    updateExportStatus('rapportExportContainer', `"${filename}" wird in der Cloud gespeichert...`, true);

    apiCall('exportRapport2Xlsx', { 
        xlsFilename: filename, 
        termin: aktTermin 
    })
        .then(function(statusMeldung) {
        updateExportStatus('rapportExportContainer', statusMeldung, false);
    })
    .catch(function(error) {
        updateExportStatus('rapportExportContainer', `Fehler beim speichernder XLSX-Datei `, false);
    });
}
